...
const ToursPage = () => {
  ...
  const mapBackendToursToDisplayFormat = (backendTours: Tour[]) => {
    return backendTours.map((tour) => ({
      ...
      percentageDiscountAllowed: tour.percentage_discount_allowed
  ...
  React.useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setTours(
          mapBackendToursToDisplayFormat(
            Array.isArray(fetchedTours) ? fetchedTours : []
          )
        );
        setDestinations(
  ...
  const handleCreateTour = async () => {
    if (!formData.name || !formData.destination || formData.capacity <= 0) {
      toast.warning(
        t("tours.fillRequiredFields") || "Please fill in all required fields"
      );
      return;
    }
    ...
    try {
      ...
      setTours(
        mapBackendToursToDisplayFormat(
          Array.isArray(updatedTours) ? updatedTours : []
        )
      );
      toast.success(
        t("tours.tourCreatedSuccessfully") || "Tour created successfully!"
      );
    } catch (error) {
      console.error("Error creating tour:", error);
      toast.error(
        t("tours.errorCreatingTour") || "Error creating tour. Please try again."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleUpdateTour = async () => {
    ...
    try {
      ...
      toast.success(
        t("tours.tourUpdatedSuccessfully") || "Tour updated successfully!"
      );
    } catch (error) {
      console.error("Error updating tour:", error);
      toast.error(
        t("tours.errorUpdatingTour") || "Error updating tour. Please try again."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDeleteTour = async (tour: any) => {
    ...
    try {
      ...
      Swal.close();
      toast.success(
        t("tours.tourDeletedSuccessfully") || "Tour deleted successfully!"
      );
    } catch (error) {
      console.error("Error deleting tour:", error);
      Swal.close();
      toast.error(
        t("tours.errorDeletingTour") || "Error deleting tour. Please try again."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleExportTours = () => {
    ...
    link.setAttribute(
      "download",
      `tours_export_${format(new Date(), "yyyy-MM-dd")}.csv`
    );
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; margin: 20px; color: #333;">
        ...
        <div style="margin-bottom: 20px; padding: 15px; background-color: #f3f4f6; border-radius: 8px; text-align: center;">
          ...
          <p><strong>Total Capacity:</strong> ${tours.reduce(
            (sum, tour) => sum + tour.capacity,
            0
          )} passengers</p>
        </div>
  ...
  React.useEffect(() => {
    if (selectedTour && showEditDialog) {
      ...
      const destinationObj = destinations.find(
        (dest) => dest.name === selectedTour.destination
      );
      const destinationId = destinationObj
        ? destinationObj.id
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div className="space-y-2 col-span-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleFormChange("capacity", parseInt(e.target.value) || 0)
                }
                className="h-10"
              />
            </div>
            ...
            <div className="space-y-2 col-span-1">
              <Label htmlFor="babyPrice" className="text-sm font-medium">
                Baby Price
              </Label>
              <Input
                id="babyPrice"
                ...
                onChange={(e) =>
                  handleFormChange("babyPrice", parseFloat(e.target.value) || 0)
                }
                className="h-10"
              />
            </div>
            <div className="space-y-2 col-span-1">
              <Label
                htmlFor="percentageDiscountAllowed"
            ...
            <div className="space-y-2 col-span-1">
              <Label htmlFor="startingPoint" className="text-sm font-medium">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleFormChange("startingPoint", e.target.value)
                }
                className="h-10"
              />
            </div>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="w-[95vw] max-w-2xl max-h-[95vh] overflow-y-auto mx-auto p-4 sm:p-6">
          ...
          {selectedTour && editFormData && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
              <div className="space-y-2 col-span-1">
                <Label htmlFor="editTourName" className="text-sm font-medium">
                  {t("tours.tourName")}
                </Label>
                <Input
                  id="editTourName"
              ...
              <div className="space-y-2 col-span-1">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleEditFormChange("destination", value)
                  }
                >
                  <SelectTrigger className="h-10">
                    <SelectValue />
              ...
              <div className="space-y-2 col-span-1">
                <Label htmlFor="editAdultPrice" className="text-sm font-medium">
                  {t("tours.adultPrice")}
                </Label>
                <Input
                  id="editAdultPrice"
              ...
              <div className="space-y-2 col-span-1">
                <Label htmlFor="editChildPrice" className="text-sm font-medium">
                  {t("tours.childPrice")}
                </Label>
                <Input
                  id="editChildPrice"
              ...
              <div className="space-y-2 col-span-1">
                <Label htmlFor="editBabyPrice" className="text-sm font-medium">
                  Baby Price
                </Label>
                <Input
                  id="editBabyPrice"
              ...
              <div className="space-y-2 col-span-1">
                <Label
                  htmlFor="editPercentageDiscountAllowed"
                  className="text-sm font-medium"
                >
                  Discount Allowed
                </Label>
              ...
              <div className="space-y-2 col-span-1">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleEditFormChange("currency", value)
                  }
                >
                  <SelectTrigger className="h-10">
                    <SelectValue placeholder="Select currency" />
              ...
              <div className="space-y-2 col-span-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("startingPoint", e.target.value)
                  }
                  className="h-10"
                />
              </div>
              ...
              <div className="col-span-full flex items-center space-x-3 py-2">
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    handleEditFormChange("active", checked)
                  }
                />
